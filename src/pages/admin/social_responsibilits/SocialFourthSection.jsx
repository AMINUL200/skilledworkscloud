...
import { Save, Plus, Edit, Trash2, X, Heart, Image, Sparkles } from 'lucide-react';
import { sub } from 'framer-motion/client';

const SocialFourthSection = () => {
  ...
  const fetchSections = async () => {
    try {
      ...
      const response = await api.get('/admin/sr-forth-section-list');

      if (response.data.status && response.data.data) {
        setSections(response.data.data);
      }
    } catch (error) {
      console.error('Error fetching SR fourth section:', error);
      setMessage({
        type: 'error',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const response = await api.delete(`/admin/sr-forth-section/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingSection) {
        // Update existing section - pass id in the URL
        // formData.id = editingSection.id; // Ensure the ID is included in the form data
        submitData.append('id', editingSection.id); // Append ID to form data for update
        response = await api.post(`/admin/sr-forth-section/save`, submitData, {
          headers: {
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
                ...
                Social Responsibility Section 4
              </h2>
              <p className="text-sm text-gray-600">Manage the impact and community development section</p>
            </div>
          </div>
        </div>
        <div className="flex justify-center items-center h-48">
          <div className="flex flex-col items-center">
            ...
            <span className="text-gray-500 mt-2">Loading sections...</span>
          </div>
        </div>
      </div>
    );
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">
                  Characters: {formData.title2.length}
                </p>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-gray-500 mt-1">When inactive, this section won't be displayed on the website</p>
              </div>
            </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="mt-4 pt-4 border-t border-green-200">
                          <h4 className="text-sm font-medium text-gray-700">{section.title2}</h4>
                          <p className="text-sm text-gray-600 mt-1">{section.desc2}</p>
                        </div>
                        
                        <div className="flex flex-wrap gap-3 mt-3">
                          {section.image_alt && (
                            <span className="text-xs text-gray-500">
                              Image Alt: {section.image_alt}
                            </span>
                          )}
                          {section.web_image && (
...
export default SocialFourthSection;