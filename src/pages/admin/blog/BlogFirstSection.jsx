...
const BlogFirstSection = () => {
  ...
  const fetchSectionData = async () => {
    try {
      ...
      const response = await api.get('/admin/blog-first-section');
      ...
      console.error('Error fetching blog first section:', error);
      if (error.status === 404 || error.response?.status === 404) {
        setIsEditing(false);
        setMessage({
          type: 'info',
          text: 'No data found. Create new blog section.'
        });
      } else {
        setMessage({
          type: 'error',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/admin/blog-first-section/save', submitData, {
        headers: {
      ...
      if (response.data.status) {
        setMessage({
          type: 'success',
          text: response.data.message || 'Blog first section saved successfully!'
        });
      ...
      console.error('Error saving blog first section:', error);
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Manage the blog page header and introduction section</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., OUR BLOG, LATEST POSTS"
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="e.g., Articles & Insights"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              disabled={saving}
            />
