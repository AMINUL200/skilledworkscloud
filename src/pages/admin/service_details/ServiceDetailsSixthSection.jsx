...
import { Save, Plus, Edit, Trash2, X, Layers, Globe, ShoppingCart, LayoutDashboard, Briefcase, MessageCircle } from 'lucide-react';

const ServiceDetailsSixthSection = () => {
  ...
  const fetchSections = async () => {
    try {
      ...
      const response = await api.get('/admin/sv-sixth-section/list');
  ...
  const handleEditService = (index) => {
    ...
    setServiceFormData({
      ...
      button_name: service.button_name || 'Learn More',
  ...
  const getIconBorderColor = (iconName) => {
    switch(iconName) {
      case 'globe':
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const response = await api.delete(`/admin/sv-sixth-section/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (key === 'services') {
          // Send each service as individual form fields with array notation
          formData[key].forEach((service, index) => {
            submitData.append(`services[${index}][icon]`, service.icon || '');
            submitData.append(`services[${index}][title]`, service.title || '');
            submitData.append(`services[${index}][description]`, service.description || '');
            submitData.append(`services[${index}][button_name]`, service.button_name || '');
            submitData.append(`services[${index}][button_url]`, service.button_url || '');
          });
      ...
      if (editingSection) {
        ...
        response = await api.post('/admin/sv-sixth-section/save', submitData, {
          headers: {
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-800 flex items-center gap-2">
                ...
                Service Details Sixth Section
              </h2>
              <p className="text-sm text-gray-600">Manage service details services section</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {!showForm && (
            <button
              ...
              className="px-4 py-2 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 flex items-center gap-2 transition-colors"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g., service_sixth_section"
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                  required
                  disabled={saving}
                />
                <p className="text-xs text-gray-500 mt-1">
                  Unique identifier for this section (e.g., service_sixth_section)
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g., For Every Business Need"
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
                  disabled={saving}
                />
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="button_url"
                      value={serviceFormData.button_url}
                      onChange={handleServiceChange}
                  ...
                  {editingServiceIndex !== null ? (
                    <>
                      <button
                        ...
                        className="px-4 py-2 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        disabled={saving}
                      >
                        Update Service
                      </button>
              ...
              {formData.services.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {formData.services.map((service, index) => (
                    <div
                      key={index}
                      className={`bg-white border ${getIconBorderColor(service.icon)} rounded-lg p-4 hover:shadow-md transition-shadow`}
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-start gap-3">
                            <div className={`p-2 rounded-lg ${getIconBgColor(service.icon)}`}>
            ...
            <div className="flex flex-wrap gap-4 pt-4 border-t border-gray-200">
              <button
                type="submit"
                disabled={saving}
                className="px-6 py-2 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-colors"
              >
                {saving ? (
                  <>
            ...
            {sections.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {sections.map((section) => (
                  <div
                    key={section.id}
                    className="bg-gradient-to-br from-cyan-50 to-blue-50 border border-cyan-200 rounded-lg p-6 hover:shadow-md transition-shadow"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        {section.batch && (
                          <p className="text-sm text-cyan-600 font-medium">{section.batch}</p>
                        )}
                        
                        <h3 className="text-xl font-semibold text-gray-800 mt-1">
                          {section.title}
                          {section.highlighted_title && (
                            <span className="text-cyan-600"> {section.highlighted_title}</span>
                          )}
                        </h3>
                        ...
                        {section.services && section.services.length > 0 && (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mt-4">
                            {section.services.map((service, index) => (
                              <div
                                key={index}
                                className={`bg-white border ${getIconBorderColor(service.icon)} rounded-lg p-3`}
                              >
                                <div className="flex items-start gap-2">
                                  <div className={`p-1.5 rounded-lg ${getIconBgColor(service.icon)}`}>
                                    {getIconComponent(service.icon)}
                                  </div>
                                  <div>
                        ...
                        <div className="mt-4 p-4 bg-white rounded-lg border border-cyan-200">
                          ...
                          {section.button_name && (
                            <div className="mt-3">
                              <span className="inline-block px-4 py-2 bg-cyan-600 text-white text-sm rounded-lg">
                                {section.button_name}
                              </span>
                            </div>
                          )}
...
export default ServiceDetailsSixthSection;