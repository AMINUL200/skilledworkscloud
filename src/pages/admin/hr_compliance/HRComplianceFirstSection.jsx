...
const HRComplianceFirstSection = () => {
  ...
  const [bgWebImage, setBgWebImage] = useState(null);
  const [bgMobileImage, setBgMobileImage] = useState(null);
  const [bgWebImagePreview, setBgWebImagePreview] = useState(null);
  const [bgMobileImagePreview, setBgMobileImagePreview] = useState(null);
  ...
  useEffect(() => {
    return () => {
      if (bgWebImagePreview && bgWebImagePreview.startsWith('blob:')) {
        URL.revokeObjectURL(bgWebImagePreview);
      }
      if (bgMobileImagePreview && bgMobileImagePreview.startsWith('blob:')) {
        URL.revokeObjectURL(bgMobileImagePreview);
      }
    };
  }, [bgWebImagePreview, bgMobileImagePreview]);
  ...
  const fetchSectionData = async () => {
    try {
      ...
      const response = await api.get('/admin/hr-compliance-first-section/details');
      ...
      if (response.data.status && response.data.data) {
        ...
        // Set image previews
        if (data.bg_web_image) {
          const url = data.bg_web_image.startsWith('http')
            ? data.bg_web_image
            : `${import.meta.env.VITE_STORAGE_BASE_URL || ''}${data.bg_web_image}`;
          setBgWebImagePreview(url);
        }
        if (data.bg_mobile_image) {
          const url = data.bg_mobile_image.startsWith('http')
            ? data.bg_mobile_image
            : `${import.meta.env.VITE_STORAGE_BASE_URL || ''}${data.bg_mobile_image}`;
          setBgMobileImagePreview(url);
        }

        setIsEditing(true);
      }
    } catch (error) {
      console.error('Error fetching HR compliance first section:', error);
  ...
  const handleImageChange = (e, type) => {
    ...
    if (file) {
      ...
      if (type === 'web') {
        setBgWebImage(file);
        setBgWebImagePreview(previewUrl);
      } else {
        setBgMobileImage(file);
        setBgMobileImagePreview(previewUrl);
      }
  ...
  const removeImage = (type) => {
    if (type === 'web') {
      ...
      setBgWebImage(null);
      setBgWebImagePreview(null);
      const input = document.querySelector('input[name="bg_web_image"]');
      if (input) input.value = '';
    } else {
      if (bgMobileImagePreview && bgMobileImagePreview.startsWith('blob:')) {
        URL.revokeObjectURL(bgMobileImagePreview);
      }
      setBgMobileImage(null);
      setBgMobileImagePreview(null);
      const input = document.querySelector('input[name="bg_mobile_image"]');
      if (input) input.value = '';
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (bgWebImage) submitData.append('bg_web_image', bgWebImage);
      if (bgMobileImage) submitData.append('bg_mobile_image', bgMobileImage);

      const response = await api.post('/admin/hr-compliance-first-section/save', submitData, {
        headers: {
      ...
      if (response.data.status) {
        setMessage({
          type: 'success',
          text: response.data.message || 'HR Compliance first section saved successfully!'
        });
      ...
      console.error('Error saving HR compliance first section:', error);
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Manage the HR compliance hero section</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="e.g., HR Compliance Services"
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              disabled={saving}
            />
          ...
          <div className="md:col-span-2 border-t border-gray-200 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                {bgMobileImagePreview && (
                  <div className="mt-3">
                    <div className="relative inline-block">
                      <img
                        src={bgMobileImagePreview}
...
export default HRComplianceFirstSection;